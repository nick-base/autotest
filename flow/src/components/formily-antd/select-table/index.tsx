...
import { observer, useFieldSchema, useField, Schema, RecursionField } from '@formily/react';
...
type ComposedSelectTable = React.FC<React.PropsWithChildren<ISelectTableProps>> & {
...
const useColumns = (sources: ObservableColumnSource[]): TableProps<any>['columns'] => {
...
export const SelectTable: ComposedSelectTable = observer((props) => {
  ...
  // readPretty Value
  const readPrettyDataSource = useFilterOptions(orderedFilteredDataSource, selected, (value, item) =>
  ...
  const onInnerChange = (selectedRowKeys: any[]) => {
    ...
    const wholeRecords = getWholeDataSource().filter((item) => selectedRowKeys.includes(item?.[primaryKey]));
