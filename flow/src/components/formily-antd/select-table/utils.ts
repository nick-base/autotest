 ...
 * @returns 完整的字段值集合
 */
const completedKeys = (flatDataSource: any[] = [], selected: any[], primaryKey: string) => {
...
const getOutputData = (
  ...
  if (valueType === 'parent') {
    ...
    outputOptions = options.filter((options) => outputValue.includes(options[primaryKey]));
...
const getUISelected = (
  ...
  let newKeys = [];
  if (valueType === 'parent') {
    const options = flatDataSource.filter((item) => keys.includes(item[primaryKey]));
    let childrenKeys = [];
    options.forEach((option) => {
      childrenKeys = [...childrenKeys, ...getTreeKeys(option.children, primaryKey)];
    });
 ...
 * @returns 是否全部选中
 */
const getCompatibleAllSelected = (selected, dataSource, usableKeys, checkStrictly, primaryKey) => {
